...
public:
    ...
    virtual void visitConstraintStmtField(ast::IConstraintStmtField *i) {
        ast::IConstraintStmtField *ic = fin(i, m_factory->mkConstraintStmtField(
            copyT<ast::IExprId>(i->getName()),
            (i->getType())?copy(i->getType()):0
        ));
        m_constraint = ic;
        // Also published as a plain ScopeChild: a foreach's iteration variable
        // is reached through its ConstraintSymbolScope's children, which are
        // copied via copy(IScopeChild). Setting only m_constraint left that
        // path returning null, so the scope copy came back null and
        // visitConstraintStmtForeach dereferenced it.
        m_sc = ic;
    }
    ...
    // Node kinds the rewrite below does not visit. Without a copy each one is
    // dropped when a generic type is specialized -- a `dist` or `soft`
    // constraint, a float field, a slice range or a target-template function
    // written inside `struct S<...>` simply would not appear in `S<int>`.
    ...
    virtual void visitConstraintStmtSoft(ast::IConstraintStmtSoft *i) override {
        m_constraint = fin(i, m_factory->mkConstraintStmtSoft(copy(i->getExpr())));
    }
    ...
    virtual void visitDistItem(ast::IDistItem *i) override {
        m_sc = fin(i, m_factory->mkDistItem(
    ...
    virtual void visitDistWeight(ast::IDistWeight *i) override {
        m_sc = fin(i, m_factory->mkDistWeight(
            i->getIs_dividing(),
            copy(i->getExpr())));
    }
