...
The paren form (`@a(x, n=y)`) is a pssparser extension covered by
test_annotations_31.py.  This module covers the standard form and the rules
around applying an annotation.
"""
import sys
...
def test_brace_and_paren_forms_produce_the_same_structure():
    """E9.1: the paren form is retained as a documented extension."""
    brace = link(DECL + '@desc_s {.desc = "x"}\ncomponent C { }\n')
    paren = link(DECL + '@desc_s(desc = "x")\ncomponent C { }\n')

    bp = annotations_of(find(brace, "C"))[0].getParameters()
    pp = annotations_of(find(paren, "C"))[0].getParameters()
    assert len(bp) == len(pp) == 1
    assert bp[0].getName().getId() == pp[0].getName().getId() == "desc"
...
def test_paren_form_still_parses():
    root = link(DECL + '@desc_s("positional")\ncomponent C { }\n')
    assert len(annotations_of(find(root, "C"))[0].getParameters()) == 1
